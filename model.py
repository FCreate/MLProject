...
class Timer(object):
    ...
    def __call__(self, step, mess='', prints=True):
        if prints and (step % self.limit != 0) and (step > 10):
            return
        message = '[%8d][%s] %s' % (step, self.hms(self.start_time), mess)
        if prints:
            print(message)
        else:
            return message
...
class MMD_GAN(object):
    ...
    def _ensure_dirs(self, folders=['sample', 'log', 'checkpoint']):
        ...
        for folder in folders:
            ...
            self.__dict__[ff] = os.path.join("", vars(self.config)[ff], self.config.suffix + self.description)
    ...
    def set_pipeline(self):
        ...
        pipe = Pipeline(self.output_size, self.c_dim, self.real_batch_size,
                        os.path.join(self.data_dir, self.dataset), with_labels=False, format=self.format,
    ...
    def set_tower_loss(self, scope, images, Generator, Discriminator, update_collection=None, labels=None):
        ...
        self.d_images_layers = self.discriminator(self.images, self.real_batch_size,  return_layers=True, update_collection=update_collection)
        self.d_G_layers = self.discriminator(self.G,  self.batch_size, return_layers=True, update_collection="NO_OPS")
